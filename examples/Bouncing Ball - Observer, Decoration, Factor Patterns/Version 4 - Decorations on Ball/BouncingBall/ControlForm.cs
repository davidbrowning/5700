    ...
    public partial class ControlForm : Form
    {
        ...
        private void createBallButton_Click(object sender, EventArgs e)
        {
            ...
            if (changeSize.Checked)
                ball = new SizeChangingDecorator() { DecoratedBall = ball, DeltaSize = 4 };

            if (changeSpeed.Checked)
                ball = new SpeedChangingDecorator() { DecoratedBall = ball, DeltaSpeed = 6 };

            if (changeDirection.Checked)
                ball = new DirectionChangeDecorator() { DecoratedBall = ball, DeltaDegrees = 60 };

            if (changeColor.Checked)
                ball = new ColorChangingDecorator() { DecoratedBall = ball, AltColor = Ball.RandomColor };
