...
namespace AppLayer.DrawingComponents
{
    ...
    public class Drawing
    {
        private static readonly DataContractJsonSerializer JsonSerializer =
                new DataContractJsonSerializer(typeof(List<Element>), new [] { typeof(Element), typeof(Relationship), typeof(RelationshipWithAllState), typeof(RelationshipExtrinsicState) });
        ...
        public void LoadFromStream(Stream stream)
        {
            ...
            lock (_myLock)
            {
                // Since only the extrinsic state is saved, recreate the full relationship objects
                foreach (var element in loadedElements)
                {
                    var tmpRelationship = element as RelationshipWithAllState;
                    if (tmpRelationship != null)
                    {
                        Relationship fullRelationship = RelationshipFactory.Instance.GetRelationship(tmpRelationship.ExtrinsicState);
                        _elements.Add(fullRelationship);
                    }
                    else
                    {
